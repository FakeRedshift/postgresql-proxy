    ...
    def listen(self, max_connections: int = 8):
        ...
        try:
            ...
            while self.running:
                ...
                if not events:
                    # LOG.debug("polling selector...") # too much noise
                    continue
...
def run(path):
    ...
    try:
        with open(path + '/' + 'config.yml', 'r') as fp:
            config_data = yaml.load(fp, Loader=yaml.Loader)
            config = cfg.Config(config_data)
    except Exception:
        logging.critical("Could not read config. Aborting.")
        exit(1)

    def ensure_directory_and_file_exist(path_to_file):
        import os
    ...
    # ensure_directory_and_file_exist(config.settings.general_log)
    logging.basicConfig(
    ...
    # ensure_directory_and_file_exist(config.settings.intercept_log)
    qlog = logging.getLogger('intercept')
    qformat = logging.Formatter('%(asctime)s : %(message)s')
    
    # file_handler
    # qhandler_file = logging.FileHandler(config.settings.intercept_log, mode = 'w')
    # qhandler_file.setFormatter(qformat)
    # qlog.addHandler(qhandler_file)
    
    qhandler_console = logging.StreamHandler()
    qhandler_console.setFormatter(qformat)
    qlog.addHandler(qhandler_console)
