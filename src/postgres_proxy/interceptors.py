...
class Interceptor:
    ...
    def _get_plugin_interceptor_function(self, interceptor):
        if not interceptor.plugin or interceptor.plugin == 'None':
            return
    ...
    def intercept(self, packet_type, data):
        if self.interceptor_config.queries is not None:
            ...
            if packet_type == b'Q':
                ...
                query = self._intercept_query(data, ic_queries)
                data = query

        if packet_type == b'':
            data = self._intercept_context_data(data, self.interceptor_config.contexts_data)
        
        return data

    def _intercept_context_data(self, data, interceptors):
        logging.getLogger('intercept').debug("intercepting context_data %s", data)

        packet_start = data[0:4]
        context_data = data[4:-1]
        entries = context_data.split(b'\x00')[:-1]
        ...
        for interceptor in interceptors:
            ...
            entries = func(self.context, entries, codec)
            logging.getLogger('intercept').debug(
                "modifying context data using interceptor %s.%s\n%s, %s",
                interceptor.plugin,
                interceptor.function,
                context_data,
                codec)
        ...
        for k, v in entries.items():
            ...
            value: str = v.decode(codec)
            self.context['connect_params'][key] = value
        ...
        return packet_start + context_data + b'\x00\x00'
...
class ResponseInterceptor(Interceptor):
    def intercept(self, packet_type, data):
        logging.getLogger('intercept').debug("Response Interceptor packet_type=%s, data=%s", packet_type, data)
    ...
    def _intercept_parameter_status(self, data, interceptors):
        ...
        logging.getLogger('intercept').debug("Response Interceptor key=%s value=%s", key, value)
        for interceptor in interceptors:
            func = self._get_plugin_interceptor_function(interceptor)
            key, value = func(self.context, key, value)
            logging.getLogger('intercept').debug(
